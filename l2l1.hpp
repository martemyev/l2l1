...
void l2l1(Parameters &param)
{
  ...
  {
    if (param._verbose > 0) std::cout << "Cross correlation:\n";

    for (int lag = -param._lag_region; lag <= param._lag_region; ++lag)
    {
      std::vector<double> xcorrelation;
      x_correlation_by_traces(data0, data1,
                              param._row_beg, param._row_end,
                              param._col_beg, param._col_end,
                              lag, xcorrelation);
  ...
  else if (param._cross_correlation == 2)
  {
    ...
    {
      double xcorrelation;
      x_correlation_whole(data0, data1,
                          param._row_beg, param._row_end,
                          param._col_beg, param._col_end,
                          lag, xcorrelation);
